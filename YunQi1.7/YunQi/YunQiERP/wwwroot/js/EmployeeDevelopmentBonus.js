...
function Orderfunction(option) {
    ...
    //9.系統顯示營運獎金計算成功。
    function CaculateEmployeeDevelopmentBonus(data) {
    ...
    //8.系統在4啟動之aax回呼函式中判斷7傳回值===0。
    //9.系統顯示營運獎金計算成功。
    function AccountEmployeeDevelopmentBonus(data) {
        data === 0 ? (alert("營運獎金入賬成功")) : ("");
        data === -1 ? (alert("執行錯誤，請稍後再試！")) : ("");
        data === -2 ? (alert("月份還在進行中！")) : ("");
        data === -3 ? (alert("已經入帳！")) : ("");
        data === -4 ? (alert("會計已經入帳！")) : ("");
    ...
    $(document).on('click', '#btnCaculate', event => {
        ...
        $.ajax({
            ...
            cache: false,
            dataType: 'Json',
            url: setting.CaculateEmployeeDevelopmentBonus,
            data: {
                'Month': jsDate,
            }
        })
            //6.系統判斷5成功執行。
            //7.系統回傳5傳回值。
            .done(CaculateEmployeeDevelopmentBonus)
            .fail(SystemError)
    });

    //20181205  ---棋
    //1.使用者在業務發展獎金管理首頁輸入日期並按獎金入帳。
    $(document).on('click', '#btnEntryAccount', event => {
        ...
        jsDate = ($('#sDate').val().substr(0, 4) + $('#sDate').val().substr(5, 2) + "01");
        console.log(jsDate); //測試用
        // 4.系統以ajax呼叫POST Action【EmployeeDevelopmentBonus / AccountEmployeeDevelopmentBonus】，並傳送jsDate
        $.ajax({
            type: 'POST',
            cache: false,
            dataType: 'Json',
            url: setting.AccountEmployeeDevelopmentBonus,
            data: {
                'Month': jsDate,
            }
        })
            ...
            .done(AccountEmployeeDevelopmentBonus)
            .fail(SystemError)
    })
    ...
    $(document).on('click', '#btnSearchMemberBonus', event => {
        ...
        $.ajax({
            ...
            cache: false,
            dataType: 'HTML',
            url: setting.GetEmployeeDevelopmentBonusList,
            data: {
    ...
    //20181205  ---棋
    //1.在業務發展獎金管理首頁點按某一筆批發會員獎金之直屬會員/隠藏。
    $(document).on('click', '.btn-detail', function (event) {
        ...
        //3.系統以ajax呼叫Post Controller Action【EmployeeDevelopmentBonus / GetEmployeeDevelopmentBonusDetailList】，並傳送jReferrerMobile、jsDate與1點按頁碼。
        $.ajax({
            type: 'Post',
            cache: false,
            dataType: 'HTML',
            url: setting.GetEmployeeDevelopmentBonusDetailList,
            data: {
    ...
    $(document).on('click', '.inner-page-link', function (event) {
        ...
        // 3.系統以ajax呼叫Post Controller Action【EmployeeDevelopmentBonus/GetEmployeeDevelopmentBonusDetailList】，並傳送jOrderId與1點按頁碼。
        $.ajax({
            type: 'Post',
            cache: false,
            dataType: 'HTML',
