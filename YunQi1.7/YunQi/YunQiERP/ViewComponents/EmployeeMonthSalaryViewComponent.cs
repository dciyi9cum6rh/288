...
namespace YunQiERP.ViewComponents
{
    //20181208  ---棋
    [Authorize]
    public class EmployeeMonthSalaryViewComponent : ViewComponent
    {
        private IEmployeeRepository IER { get; set; }
        private IProductRepository IPR { get; set; }
        private IConfiguration config { get; set; }

        public EmployeeMonthSalaryViewComponent(IConfiguration configuration, IEmployeeRepository EmployeeRepository, IProductRepository ProductRepository)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync(string EmployeeMobile, int Month, int Page = 1, int LinkType = 0, int StartPage = 1, string AClass = "inner-page-link")
        {
            ...
            if (EmpCount % RowsPerPage > 0) TotalPages += 1;
            if (Page < 1) Page = 1;
